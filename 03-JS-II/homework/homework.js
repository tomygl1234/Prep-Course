...
function mayoriaDeEdad(edad) {
  ...
  if(edad >= 18){return "Allowed"} else if (edad < 18){return "Not allowed"}  
}
...
function conection(status) {
  ...
  if(status !== 1 && status !== 2){return "Offline"}
...
function esEntero(numero) {
  ...
  // Tu código:
  if(numero === Math.floor(numero)){return true} else {return false}
}
...
function operadoresLogicos(num1, num2, num3) {
  ...
  if(num2 === 0 || num3 === 0 || num1 === 0){return "Error"}
  if(num1 < 1 || num2 < 1 || num3 < 1){return "Hay negativos"}
  if(num1 > 1 && num1 > num2 && num1 > num3){return "Número 1 es mayor y positivo"}
...
function tablaDelSeis(){
  ...
  //Escribe tu código aquí
  var tabla = []
  for(var i = 0; i < 11; i++){tabla.push( i * 6 )}
  return tabla
}
